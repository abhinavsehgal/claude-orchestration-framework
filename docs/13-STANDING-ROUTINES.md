...
(Chapter 2), hooks enforce discipline while it runs (Chapter 10), and the truth files brief the next
session (Chapter 11) — but nothing happens between sessions. This chapter adds the third leg:
**routines** — narrow agent jobs that run on a schedule, produce small reviewable pull requests, and
get *tuned* instead of babysat.

The pattern is not speculative. The Claude Code team ran it publicly on their own repos in Aug 2026:
a fleet of daily routines (crash fuzzing, duplicate-abstraction unification, dead-code removal,
flaky-test root-causing) opened **388 PRs in a few weeks, 180 of which were merged** after automated
review plus human review, with roughly 1 in 50 PRs being noise. Everything below restates that
...
version, rotate a log), use plain automation; a routine earns its cost only where the work needs
reading, reasoning, or writing code.
...
integration branch only through a PR (Pitfall 11 applies doubly to unattended writers). Small and
self-contained is what makes ten routine PRs a five-minute review instead of an afternoon.
...
**4. All routine activity reports to one place.** One channel, one issue thread, or one dated log
...
than no routines (see Pitfall 28).

**5. A routine never merges its own PR.** The gate is: automated review first (cheapest tier on
every PR; a stronger review tier for anything touching money, auth, data deletion, or another
repo's contract), then a human merge. The human's cost stays low *because* of conventions 2 and 3.
...
(Chapter 10, Pattern 2) applied to unattended work. Expect a real noise rate — the public
reference fleet ran at ~1 in 50 — and budget it; a routine whose noise stays above its budget after
two tuning passes gets retired, not tolerated.

**7. Attempt caps and a verified retire path.** Every routine carries a per-run budget (time,
turns, or tokens), an attempt cap after which it parks instead of grinding, and — critically — a
*checked* completion write. One production team's generator pipeline ran for 17 days without a
...
read**, and "the routine ran" must never be reported as "the routine worked" (Pitfall 12, unattended
edition).

## Model and effort tiering

Match the model to the routine, not the fleet. The public reference fleet ran mostly on the
mid-size frontier model, reserving the largest for a few hard routines — and noted that a smaller
model is workable if you "spend a bit more time auditing PRs and iterating on routines' prompts,
then adding in checks and guardrails." That trade is general: **cheaper model ⇒ tighter output
contract, more automated checks, more sampling in review.** Set effort/budget per routine, and
re-tier quarterly as models change (Pitfall 18).
...
- **Doc-drift checker** — diff Tier-1/Tier-2 docs (Chapter 7) against reality: dead links, stale
  counts, commands that no longer exist. Companion to the librarian (Pitfall 14), running daily
...
- **Workspace janitor** — prune stale worktrees, merged branches, dead clone directories
  (Pitfall 24's cleanup crew).
- **Hill-climber** — a routine wrapper around the hill-climb skill
  (`templates/hill-climb-skill.md.template`): one metric, one target, one bounded iteration per run.

## Running them on Claude Code

Three mechanisms, in order of preference:

1. **Product-native schedulers** — Claude Code's routines/scheduled-task surfaces (verified to
   exist 2026-08-24; the product is moving fast here, so re-verify capabilities rather than trusting
   this page). Least infrastructure, best observability.
2. **CI cron + headless `claude -p`** (Mode 4, Chapter 6) — the portable floor that works on any
   scheduler. Note the verified behavior (2026-08-22): `claude -p` **runs the working directory's
   hooks**, so a routine executing in a child repo gets that repo's enforcement for free — this is
   the same property Chapter 12 relies on for cross-repo delegation, and it is why routines should
   `cd` into the repo they operate on.
3. **An agent teammate in a chat channel** (e.g. Claude in Slack) — when the fleet's reporting
   channel and its trigger surface should be the same place.

Give every mechanism a kill switch a human can flip without a deploy (an env var, a channel
command, disabling the workflow) — you will want it during an incident.
...
- Don't give a routine a broad toolset "to be safe" — charter-narrow tools, same as any specialist
  (Principle 5); an unattended agent with wide write access is your largest blast radius.
- Don't run a routine without a budget and an attempt cap — a runaway loop discovered on the
  monthly bill is the canonical failure (see also REFINEMENT check 11).
...
- `templates/hill-climb-skill.md.template` — the metric-loop skill routines can wrap.
- `docs/06-INVOCATION-MODES.md` § Mode 6 — where routines sit among the invocation modes.
- `docs/10-HOOK-HARDENING.md` — Pattern 2 (correction-capture) is the tuning loop's interactive twin.
...
- `docs/08-COMMON-PITFALLS.md` — Pitfalls 27 (context weight) and 28 (unattended jobs need a verified retire path).
