...
> codebase with many parallel agent sessions. Everything here is tech-stack and domain agnostic.

The v1.0 framework answered *"who does the work and how do they talk to each other?"* (agents,
handoff schema, rules). Three months of real use exposed the next failure class: **a fresh agent
with no transcript knows only what is in git — and git mostly recorded *code*, not *truth*.**
...
- A "we'll do that later" spoken at the end of a session vanished, and was re-discovered as a bug
  two weeks on.
- A rule file documented the wrong HTTP verb for a route. Three agents trusted it. The fourth read
  the route.
...
These sit alongside the stores v1.0 already had: `CLAUDE.md` (router), `.claude/rules/*.md`
(path-scoped hard rules), `docs/ai-context/<area>.md` (orientation), `docs/<UPPERCASE>.md`
(canonical reference), and the tool's own auto-memory.
...
|---|---|
| Universal behaviour + routing | `CLAUDE.md` (keep under ~200 lines) |
...
| A correction the owner had to give more than once | `LEARNINGS.md` §D **and** a rule file if path-scoped |
| A hard rule scoped to file paths | `.claude/rules/<domain>.md` |
| Per-area orientation (50–150 lines) | `docs/ai-context/<area>.md` |
| Deferred work | `docs/<AREA>_BACKLOG.md` |
| A multi-step repeatable procedure | `.claude/skills/<name>/SKILL.md` |
| Machine-private session pointers | Auto-memory — **not** visible to other machines or agents; anything durable must *also* land in one of the above |
...
3. Any affected `.claude/rules/*.md` hard rules.
...
now a Stop hook — `templates/hooks/doc-freshness-gate.mjs.template`, Pattern 5 in
`docs/10-HOOK-HARDENING.md` — which refuses to end a turn in which a production push happened and
`docs/CHANGELOG.md` was not touched afterwards. Teams without hooks encode the same list in the
orchestrator's Definition of Done and in the `/commit-push-pr`-style command.

---

## The engineering playbook — six gates and the evidence ladder

Template: `templates/engineering-playbook-skill.md.template`. A project-wide skill that every
substantial task runs through; the per-task-type skills (investigate-bug, build-feature) encode the
checklists, this one encodes the gates they share. **If a gate conflicts with a hard rule, the rule
wins.**
...
| 6 Report | Files, rules read, checks run **with actual results**, risks and what was *not* verified (with its confidence class), docs updated, follow-ups written to a backlog |
...
| `documented-unverified` | A doc says so — including *this project's own rule files* — and you didn't check |
...
- **Corrections harden into rules, never into "I'll remember".** Memory is machine-local and
...
- `docs/10-HOOK-HARDENING.md` — Pattern 5 (doc-freshness gate) enforces the production-push rule.
- `docs/12-MULTI-REPO-WORKSPACES.md` — the parity rules above applied across repositories.
- `docs/08-COMMON-PITFALLS.md` — Pitfalls 18–26 are the incident reports behind this chapter.
- `templates/PROJECT.md.template`, `templates/LEARNINGS.md.template`,
  `templates/BACKLOG.md.template`, `templates/engineering-playbook-skill.md.template`.
